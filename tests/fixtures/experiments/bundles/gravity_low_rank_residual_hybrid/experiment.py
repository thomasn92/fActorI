"""Deterministic gravity plus low-rank residual hybrid experiment."""
...
def _rank_one(matrix: list[list[float]]) -> tuple[list[float], list[float], float]:
    n_rows = len(matrix)
    n_cols = len(matrix[0])
...
def _run_setting(config: dict[str, object], setting: dict[str, object]) -> dict[str, object]:
    ...
    for origin in range(n_regions):
        for destination in range(n_regions):
            ...
            residual = strength * origin_factor[origin] * destination_factor[destination]
            epsilon = rng.gauss(0.0, noise_level)
            flow = (
                origin_mass[origin]
                * destination_attractiveness[destination]
                * distance ** (-alpha[origin])
                * math.exp(residual + epsilon)
            )
            rows.append(
                {
                    "origin": origin,
                    ...
                    "test": (origin * 17 + destination * 31) % 5 == 0,
                }
            )
    ...
    for row in train:
        ...
        baseline = scale * distance ** (-origin_alpha[origin])
    ...
    return {
        ...
        "parameter_count_baseline": n_regions,
        "parameter_count_method": n_regions + 2,
...
def main() -> None:
    ...
    high = next(row for row in table if row["setting"] == "high_residual_structure")
    support = all(
    ...
    metrics = {
        ...
        "scope": "synthetic gravity plus low-rank residual hybrid comparison for one mapped claim",
    }
    ...
    print("gravity low-rank residual hybrid experiment completed")
